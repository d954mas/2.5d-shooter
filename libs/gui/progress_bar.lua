...
function Bar:gui_update()
	...
	gui.set_size(self.vh.progress, vmath.vector3(self.progress_width_max * self.animation.value / self.value_max,
			gui.get_size(self.vh.progress).y, 0))
end
...
function Bar:set_value(value)
	...
	self.animation.tween = ACTIONS.Tween { object = self.animation, property = "value", from = { value = self.value },
