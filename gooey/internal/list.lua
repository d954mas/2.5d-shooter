...
function M.dynamic(list_id, stencil_id, item_id, data, action_id, action, fn, refresh_fn, set_parent)
	...
	if not list.items then
		...
		for i=1,item_count do
			...
			if set_parent then gui.set_parent(list.items[i].root,list.stencil) end
